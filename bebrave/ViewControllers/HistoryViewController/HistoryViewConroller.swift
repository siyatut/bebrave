...
class HistoryViewController: UIViewController {
    ...
    private func calculateProgress(for period: Period) {
        // TODO: - Опять что-то неправильно здесь посчитывается. Или UI некорректно обновляется. Только при перезагрузки, а нужно чтобы и при смене экрана апдейтился. С полгода опять что-то не так. С месяцем тоже проблемы...
