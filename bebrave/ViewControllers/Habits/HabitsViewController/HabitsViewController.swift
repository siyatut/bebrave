...
#warning("№1: Нужно будет ещё добавить условия для пропуска привычки, когда по каким-то причинам пользователь не хочет выполнять её + условия, при котором она будет считаться невыполненной без его нажатия — после 00 по таймзоне юзера")

#warning("№2: Кнопку «Пропустить сегодня» добавить во вью контроллер «Изменить привычку»")
