...
#warning("№1: Переделать swipe gesture по типу реализации в Telegram. Убрать реализацию именно свайпа, мб кроме удаления. Посмотреть по ситуации. Слева сделала: «Изменить», «Отменить», а справа: «Пропустить» и «Удалить». UPD: Почти сделала, но есть проблема: ячейка не закрашивается в жёлтый при пропуске — надо переписать метод configure(with habit: Habit)")
