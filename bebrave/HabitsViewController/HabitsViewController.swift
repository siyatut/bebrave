...
#warning("№2: Переписать отрисовку серого пустого вью на коллекцию (пусть и закроет ячейку дневника), чтобы после удаления последней привычки не возникало той же самой ошибки. UPD: переписала, но оно закрывает и кнопку добавить привычку, потому что та является footer секции. Отказаться от compositional layout, чтобы нормально организовать код?")
